...
pub enum VECommandBufferError {
    ...
    #[error("command pool locking failed")]
    CommandPoolLockingFailed,
}

pub struct VECommandBuffer {
    device: Arc<VEDevice>,
    command_pool: Arc<Mutex<VECommandPool>>,
                ...
                .allocate_command_buffers(
                    &CommandBufferAllocateInfo::default()
                        ...
                        .command_pool(
                            command_pool
                                .lock()
                                .map_err(|_| VECommandBufferError::CommandPoolLockingFailed)?
                                .handle,
                        ),
                )
...
impl Drop for VECommandBuffer {
    fn drop(&mut self) {
        let command_pool = self.command_pool.lock();
        if let Ok(command_pool) = command_pool {
            unsafe {
                self.device
                    .device
                    .free_command_buffers(command_pool.handle, &[self.handle])
            };
        }
    }
}
