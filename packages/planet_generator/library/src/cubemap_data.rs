...
impl<Data: Clone> CubeMapDataLayer<Data> {
    ...
    pub fn set_face(&mut self, face: &CubeMapFace, value: Vec<Data>) {
        match face {
            CubeMapFace::PX => self.px = Arc::new(Mutex::new(value)),
            CubeMapFace::PY => self.py = Arc::new(Mutex::new(value)),
            CubeMapFace::PZ => self.pz = Arc::new(Mutex::new(value)),
            CubeMapFace::NX => self.nx = Arc::new(Mutex::new(value)),
            CubeMapFace::NY => self.ny = Arc::new(Mutex::new(value)),
            CubeMapFace::NZ => self.nz = Arc::new(Mutex::new(value)),
        }
    }

    pub fn set_index(&self, face: &CubeMapFace, index: usize, value: Data) {
        match face {
