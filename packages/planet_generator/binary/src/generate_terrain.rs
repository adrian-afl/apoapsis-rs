...
use std::path::Path;
use universe_simulation::body_definitions::{
    BodyBiomeModifier, BodyCelestialBodyDefinition, BodyTerrain,
};

fn generate_biomes(
    input: &BodyCelestialBodyDefinition,
    terrain: &BodyTerrain,
    cube_map_height: &CubeMapDataLayer<f64>,
    cube_map_biome: &CubeMapDataLayer<InterpolatedBiomeData>,
) {
    let cube_map_res = input.generator_config.as_ref().unwrap().cube_map_resolution;
        ...
        for y in (0..cube_map_res) {
            for x in (0..cube_map_res) {
                ...
                let modifier = match terrain.biome_modifier {
                    BodyBiomeModifier::Latitude => dir.y.abs() * 90.0,
                    BodyBiomeModifier::Tidal => -dir.z, // so by default -z faces the star
                    BodyBiomeModifier::Random => fbm(dir, 4, 2.0, 0.5),
                };
...
pub fn generate_terrain(out_dir: &str, input: &BodyCelestialBodyDefinition) {
    ...
    erosion_run(
        &mut cube_map_height,
        &mut cube_map_biome,
        generator_config.erosion_iterations,
        generator_config.erosion_droplets_count,
        terrain.radius,
        generator_config.erosion_droplet_velocity_coefficient,
        generator_config.erosion_droplet_evaporation_coefficient,
    );
