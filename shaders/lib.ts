...
export async function compileFile(entrypoint: string): Promise<string | null> {
    ...
    if (compilationResult.status !== 0) {
        return `Errors in ${entrypoint}:\n` + compilationResult.stderr.toString("utf-8").trim();
    }
    return null;
}
...
export async function compileAll() {
    ...
    const results = await Promise.all(
        entrypoints.map((entrypoint) => compileFile(entrypoint)));
    for (const result of results) {
        if (result) {
            console.log(util.styleText("red", result));
        }
    }
}